              ...
              void *user_data)
{
    ...
    // 添加前缀标识这是蓝牙数据
    pos = snprintf(hex_message, sizeof(hex_message), "BLE:");
    
    // 转换每个字节为十六进制
    for (size_t i = 0; i < len && pos < (BUFFER_SIZE - 4); i++) {
        snprintf(temp_str, sizeof(temp_str), "%02X", value[i]);
        if (pos + 2 < BUFFER_SIZE - 1) {
            hex_message[pos++] = temp_str[0];
            hex_message[pos++] = temp_str[1];
            if (i < len - 1 && pos < BUFFER_SIZE - 1) {
                hex_message[pos++] = ' '; // 添加空格分隔
            }
        }
    }
    hex_message[pos] = '\0';
    ...
    // 通过共享内存发送数据
    send_to_display(hex_message);
    
    // 必须响应客户端（表示写入成功）
    gatt_db_attribute_write_result(attrib, id, 0);
}

static void upload_service(struct server *server)
{
    bt_uuid_t uuid;
    struct gatt_db_attribute *service, *upload_char;

    // 创建上传服务
    bt_uuid16_create(&uuid, UPLOAD_SERVICE_UUID);
    service = gatt_db_add_service(server->db, &uuid, true, 3);

    // 添加上传特征（只写）
    bt_uuid16_create(&uuid, UPLOAD_CHARACTERISTIC_UUID);
    upload_char = gatt_db_service_add_characteristic(service, &uuid,
            BT_ATT_PERM_WRITE,  // 只允许写入
            BT_GATT_CHRC_PROP_WRITE,  // 只设置写属性
            NULL,  // 无读回调
            upload_write_cb,  // 使用全局定义的写回调
            server);
...
int main(int argc, char *argv[])
{
	...
	// 初始化IPC通信
	if (init_ipc() != 0) {
		fprintf(stderr, "Failed to initialize IPC communication\n");
		close(fd);
		return EXIT_FAILURE;
	}
